        ...
        {renderItems.map((item) => {
          ...
          return (
            <MessageBlock
              ...
              onDeleteMessage={
                resolvedSessionId
                  ? (messageId: string) => deleteMessage(resolvedSessionId, messageId)
                  : undefined
              }
              onToggleBookmark={
                resolvedSessionId
                  ? (messageId: string) => toggleBookmark(resolvedSessionId, messageId)
                  : undefined
              }
              isBookmarked={
                resolvedSessionId
                  ? (bookmarkedMessages[resolvedSessionId] || new Set()).has(msg.id)
                  : false
              }
              onRetryMessage={
                resolvedSessionId && !isMemberSession
                  ? (messageId: string) => retryMessage(resolvedSessionId, messageId)
                  : undefined
              }
            />
          )
...
export const MessageBlock = memo(function MessageBlock({
  ...
  switch (message.type) {
    case 'user_text':
      return (
        <UserMessage
          ...
          onBookmark={onToggleBookmark ? () => onToggleBookmark(message.id) : undefined}
    ...
    case 'error': {
      ...
      return (
        <div className="group mb-3 px-4 py-2.5 rounded-lg border border-[var(--color-error)]/20 bg-[var(--color-error-container)]/28 text-sm text-[var(--color-error)]">
          <div className="flex items-start justify-between gap-2">
            ...
            {onRetryMessage && (
              <button
                ...
                className="shrink-0 inline-flex items-center gap-1 rounded-full border border-[var(--color-error)]/30 bg-[var(--color-surface-container-low)] px-2.5 py-1 text-[11px] font-medium text-[var(--color-error)] transition-colors hover:bg-[var(--color-error)]/10"
                title="重试"
              >
