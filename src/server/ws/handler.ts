...
async function handleSetRuntimeConfig(
  ...
  const nextOverride = {
    ...
    enabledMcpServers: message.enabledMcpServers,
  ...
  const configChanged =
    ...
    JSON.stringify(prevOverride.enabledSkills) !== JSON.stringify(nextOverride.enabledSkills) ||
    JSON.stringify(prevOverride.enabledMcpServers) !== JSON.stringify(nextOverride.enabledMcpServers) ||
    JSON.stringify(prevOverride.enabledAgents) !== JSON.stringify(nextOverride.enabledAgents) ||
    JSON.stringify(prevOverride.enabledPlugins) !== JSON.stringify(nextOverride.enabledPlugins)
...
function translateCliMessage(cliMsg: any, sessionId: string): ServerMessage[] {
  ...
  switch (cliMsg.type) {
    ...
    case 'result': {
      ...
      // Record usage statistics for non-proxy path (SDK direct calls)
      if (usage.input_tokens > 0 || usage.output_tokens > 0) {
        const cost = estimateCost(usage.input_tokens, usage.output_tokens, cliMsg.model || '')
        addUsage(usage.input_tokens, usage.output_tokens, cost)
      }
    ...
    case 'system': {
      ...
      if (subtype === 'init') {
        ...
        const messages: ServerMessage[] = [
          ...
          { type: 'system_notification', subtype: 'init', message: `Model: ${cliMsg.model || 'unknown'}`, data: { model: cliMsg.model, provider: cliMsg.provider } },
        ]
...
async function getRuntimeSettings(sessionId?: string): Promise<{
  ...
  if (runtimeOverride) {
    ...
    return {
      ...
      enabledSkills: runtimeOverride.enabledSkills,
      enabledMcpServers: runtimeOverride.enabledMcpServers,
      enabledAgents: runtimeOverride.enabledAgents,
      enabledPlugins: runtimeOverride.enabledPlugins,
    }
  }
