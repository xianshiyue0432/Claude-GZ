...
import { buildFileTreeFromPath } from '../components/chat/DraggableTerminalWindow'
import type { FileNode } from '../components/chat/DraggableTerminalWindow'
...
import { FigmaModule } from '../components/modules/FigmaModule'
...
export function PopoutStudio() {
  ...
  useEffect(() => {
    const onMouseMove = (e: MouseEvent) => {
      ...
      if (ds.current.mode === 'v-split') {
        ...
        const contentH = Math.max(200, containerH)
  ...
  const beginDrag = useCallback((mode: typeof ds.current.mode, extra?: Partial<typeof ds.current>) => {
    ...
    document.body.style.cursor = mode === 'v-split' ? 'col-resize' : 'row-resize'
  ...
  const handleHSplitDown = useCallback((e: React.MouseEvent) => {
    e.preventDefault()
    beginDrag('h-split', { startX: e.clientX, startY: e.clientY, startEditorPct: editorHeightPct })
  ...
  useEffect(() => {
    ...
    async function initXtermForTid(tid: number) {
      ...
      try {
        const result = await terminalApi.spawn({ cols: terminal.cols, rows: terminal.rows, cwd: workspacePath || undefined })
  ...
  const openFileInEditor = useCallback(async (node: FileNode) => {
    ...
    const nf: OpenFile = {
      ...
      content, language: langMap[ext]||'Plain Text', modified: false,
    }
    setOpenFiles(p => [...p, nf])
  ...
  const handleNewFile = useCallback(() => {
    ...
    const nf: OpenFile = {
      id: `${Date.now()}-new`, name, path: `/${name}`,
      content: '', language: 'Plain Text', modified: false,
    }
  ...
  const handleSave = useCallback(async (fileId?: string) => {
    ...
    try {
      ...
      setOpenFiles(p => p.map(f => f.id === targetId ? { ...f, modified: false } : f))
    } catch {
      setOpenFiles(p => p.map(f => f.id === targetId ? { ...f, modified: false } : f))
    }
  ...
  const handleSelectFile = useCallback(async () => {
    try {
      ...
      try {
        ...
        const nf: OpenFile = {
          id: `${Date.now()}-${Math.random().toString(36).slice(2,7)}`,
          name: fileName, path: pathStr, content, language: langMap[ext]||'Plain Text', modified: false,
        }
        ...
        const nf: OpenFile = {
          ...
          content: `// Loading ${fileName}...\n`, language: langMap[ext]||'Plain Text', modified: false,
        }
        setOpenFiles(p => [...p, nf]); setActiveFileId(nf.id)
      }
  ...
  function renderOtherModuleView() {
    if (activeModuleType === 'browser') {
      return <BrowserModule workDir={workspacePath} />
    }
    if (activeModuleType === 'document') {
      return <DocumentModule workDir={workspacePath} onOpenFile={(path) => openFileInEditor({ name: path.split(/[\\/]/).pop() || path, path, type: 'file' } as FileNode)} />
    }
    if (activeModuleType === 'git') {
      return <GitModule workDir={workspacePath} />
    }
    if (activeModuleType === 'figma') {
      return <FigmaModule workDir={workspacePath} />
    }
    ...
    return (
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 bg-[#131128]">
        ...
        <button className="mt-2 rounded-lg border border-indigo-600/40 bg-indigo-600/15 px-4 py-1.5 text-[11px] text-indigo-300 hover:bg-indigo-600/25 transition-colors">
          即将推出...
        </button>
      </div>
    )
  ...
  return (
    <div className="flex h-screen w-screen flex-col bg-[#131128] text-gray-200">
      {/* TITLE BAR */}
          ...
          <div style={{
            ...
            <div className="flex shrink-0 items-center gap-2 border-b border-[#2d2652] bg-gradient-to-r from-[#1e1850] to-[#211b45] px-3 py-1">
              ...
              <div className="ml-auto flex items-center gap-1.5">
                ...
                <button data-no-drag onMouseDown={e=>e.stopPropagation()} onClick={handleRefresh} className={`flex h-5 w-5 items-center justify-center rounded text-white text-[12px] transition-colors ${activeFile?'bg-sky-700/50 hover:bg-sky-600/60 active:scale-90':'bg-sky-700/30 hover:bg-sky-600/40'}`} title="刷新">&#8635;</button>
                <button data-no-drag onMouseDown={e=>e.stopPropagation()} onClick={()=>handleSave()} className={`flex h-5 w-5 items-center justify-center rounded text-white text-[12px] transition-colors ${activeFile?.modified?'bg-green-700/50 hover:bg-green-600/60 active:scale-90 animate-pulse':'bg-green-700/30 hover:bg-green-600/40'}`} title="保存">&#128190;</button>
                <button data-no-drag onMouseDown={e=>e.stopPropagation()} onClick={handleEditorClose}
                  className="flex h-5 w-5 items-center justify-center rounded bg-red-500/70 text-white hover:bg-red-400/80 text-[10px] font-bold transition-colors" title="关闭编辑器">&#10005;</button>
              </div>
            </div>
            ...
            <div className="min-h-0 flex-1 overflow-hidden bg-[#0c0a18] p-0">
              {activeFile ? (
                <MonacoEditorPanel
                  ...
                  onFileSave={(_fileId) => handleSave(_fileId)}
                  onFileClose={(fileId) => {
                    setOpenFiles(prev => {
                      const next = prev.filter(f => f.id !== fileId)
                      if (fileId === activeFileId) {
                        const nextActive = next.length > 0 ? next[next.length - 1] : undefined
                        setActiveFileId(nextActive ? nextActive.id : '')
                      }
                      return next
                    })
                  }}
