...
import { filterByEnabledList, isEnvTruthy } from '../envUtils.js'
import {
  errorMessage,
  getErrnoPath,
  isENOENT,
...
async function assemblePluginLoadResult(
  ...
  const filteredEnabled = filterByEnabledList(enabledPlugins, (p) => p.source, 'CC_HAHA_ENABLED_PLUGINS')
  const filteredAll = filterByEnabledList(allPlugins, (p) => p.source, 'CC_HAHA_ENABLED_PLUGINS')

  // 3. Cache plugin settings for synchronous access by the settings cascade
  cachePluginSettings(filteredEnabled)

  return {
    enabled: filteredEnabled,
    disabled: filteredAll.filter(p => !p.enabled),
    errors: allErrors,
  }
}
