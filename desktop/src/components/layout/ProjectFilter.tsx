...
export function ProjectFilter({ variant = 'default' }: { variant?: 'default' | 'embedded' }) {
  ...
  const handleChooseFolder = useCallback(async () => {
    try {
      ...
      if (activeTabId) {
        ...
        const newId = await createSession(dirPath)
        ...
        useSessionStore.getState().deleteSession(oldId).catch(() => {})
      } else {
        await createSession(dirPath)
      }
    } catch {}
  }, [])
  ...
  const handleOpenSaveDir = useCallback(async () => {
    ...
    try {
      ...
      // 使用系统默认方式打开文件夹
      await invoke('open_path', { path: saveDir })
    } catch (error) {
        ...
        >
          ...
          <div className="border-t border-[var(--color-border)] p-2">
            <button
              ...
              className="flex w-full items-center gap-3 rounded-[12px] px-3 py-2.5 text-left transition-colors hover:bg-[var(--color-sidebar-item-hover)]"
            >
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="flex-shrink-0 text-[var(--color-text-tertiary)]">
            ...
            >
              ...
              <span className="text-sm text-[var(--color-text-secondary)]">{t('sidebar.openSaveDir')}</span>
            </button>
          </div>
        </div>,
        document.body,
