...
type ChatStore = {
  ...
  bookmarkedMessages: Record<string, Set<string>>
...
const BOOKMARKS_STORAGE_KEY = 'cc-haha-bookmarked-messages'

let _bookmarksCache: Record<string, Set<string>> | null = null

function loadBookmarks(): Record<string, Set<string>> {
  ...
  try {
    ...
    if (!raw) { _bookmarksCache = {}; return _bookmarksCache }
...
function persistBookmarks(bookmarkedMessages: Record<string, Set<string>>) {
  _bookmarksCache = bookmarkedMessages
  if (typeof window === 'undefined' || typeof localStorage === 'undefined') return
  try {
    const serializable: Record<string, string[]> = {}
    for (const [key, set] of Object.entries(bookmarkedMessages)) {
      serializable[key] = Array.from(set)
    }
    localStorage.setItem(BOOKMARKS_STORAGE_KEY, JSON.stringify(serializable))
  } catch {
    // noop
  }
}
...
export const useChatStore = create<ChatStore>((set, get) => ({
  ...
  toggleBookmark: (sessionId, messageId) => {
    set((s) => {
      const current = s.bookmarkedMessages[sessionId] || new Set<string>()
      const next = new Set(current)
      if (next.has(messageId)) { next.delete(messageId) } else { next.add(messageId) }
      const updated = { ...s.bookmarkedMessages, [sessionId]: next }
      persistBookmarks(updated)
      return { bookmarkedMessages: updated }
    })
  },
  ...
  retryMessage: (sessionId, messageId) => {
    ...
    for (let i = msgIndex - 1; i >= 0; i--) {
      ...
      if (m && m.type === 'user_text') {
        userContent = m.content
        break
      }
    }
    if (!userContent.trim()) return
    set((s) => ({ sessions: updateSessionIn(s.sessions, sessionId, (sess) => ({
      messages: sess.messages.filter((m) => m.id !== messageId),
    })) }))
    wsManager.send(sessionId, { type: 'user_message', content: userContent })
  },

  bookmarkedMessages: loadBookmarks(),
    ...
    switch (msg.type) {
      ...
      case 'system_notification':
        ...
        if (msg.subtype === 'init' && msg.data && typeof msg.data === 'object') {
          const initData = msg.data as Record<string, unknown>
          const model = typeof initData.model === 'string' ? initData.model : undefined
          const provider = typeof initData.provider === 'string' ? initData.provider : undefined
          if (model) {
            update(() => ({ currentModel: model, currentProvider: provider }))
          }
        }
